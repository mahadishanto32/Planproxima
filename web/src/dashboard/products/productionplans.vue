                        ...
                        <div class="col-sm-11">
                            <div class="breadcrumb-wrapper col-9">
                                <ol class="breadcrumb p-0 mb-0">
                                    ...
                                    <li class="breadcrumb-item active"> Production Plans
                                    </li>
                                </ol>
                            </div>
                        </div> 
            ...
            <div class="content-body">
                                    ...
                                    <div class="card-body card-dashboard">
                                      ...
                                      <div class="btn-group mb-1" role="group" aria-label="Basic example">
                                        ...
                                        <button type="button" class="btn btn-primary add-btn btn-lg d-flex align-items-center">
                                          <a class="text-white" @click="show_pop()" > <i class="bx bx-add-alt"></i>Production Plans Upload </a>
                                        </button>
                                        <button  type="button"  
                                            ...
                                            <table class="table table-striped"> 
                                                    ...
                                                    <tbody>
                                                      <tr  v-for="(item , index ) in items" :key="item.id" > 
                                                        <td> {{ index + 1 }}</td>   
                                                        <td >{{ item.projoin ? item.projoin.description  :'' }}</td>  
                                                        <td >{{ item.summary_groupjoin ? item.summary_groupjoin.description  :'' }}</td>  
                                                        <td> {{ Number(item.jan).toFixed(2) }}</td>
                                                        <td> {{ Number(item.feb).toFixed(2) }}</td>
                                                        <td> {{ Number(item.mar).toFixed(2) }}</td>
                                                        <td> {{ Number(item.apr).toFixed(2) }}</td>
                                                        <td> {{ Number(item.may).toFixed(2) }}</td>
                                                        <td> {{ Number(item.jun).toFixed(2) }}</td>
                                                        <td> {{ Number(item.jul).toFixed(2) }}</td>
                                                        <td> {{ Number(item.aug).toFixed(2) }}</td>
                                                        <td> {{ Number(item.sep).toFixed(2) }}</td>
                                                        <td> {{ Number(item.oct).toFixed(2) }}</td>
                                                        <td> {{ Number(item.nov).toFixed(2) }}</td>
                                                        <td> {{ Number(item.dec).toFixed(2) }}</td>  
                                                        <td> {{ item.type }}</td> 
                                                        <td> {{ item.year }}</td> 
                                                      </tr> 
                                                    </tbody>
                ...
                <modal width="60%" height="30%" style="padding:50px" name="popup-singel">
                    ...
                    <div class="app-content ">
                       <div class="card">
                        <div class="col-sm-6"> 
                            <a class="btn-block glow users-list-clear mb-0 download_template"  >Production Plans Data Upload</a>
                            <br>
                        </div>
...
export default {
    ...
    methods: { 
        demoDownload(){
            window.location.href = window.location.origin + "/demo/production_plan_fomat.xlsx"
        },
        async getFactorys() {
            let where = '?';   
            await axios
        ...
        async getItems() {
            if(this.filterForm.summary_group_id){
                ...
                try {
                    await axios
                        .get(this.api_url + "production_plans" + where, {
                            headers: {
                                "Content-Type": "application/json",
                                Authorization: this.token ? `Bearer ${this.token}` : ""
                            },
        ...
        csvUpload() {
            if (this.headId != "") {
                ...
                let loader = this.$loading.show();
                axios
                    .post(this.api_url + "production_plans-file-upload", formData, {
                        headers: {
