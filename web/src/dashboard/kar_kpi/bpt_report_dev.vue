                            ...
                            <div class="row border rounded py-2 mb-2">
                                ...
                                <div  class="col-12 col-sm-6 col-lg-2">
                                  ...
                                  <div  class="col-12 col-sm-6 col-lg-2">
                                    ...
                                    <fieldset class="form-group">
                                      <select class="form-control" v-model="filterForm.month"   v-on:change="monthChange()"  id="users-list-verified">
                                        <option value="">All</option>
                                                    ...
                                                    <tbody>
                                                        <template v-for="(item , index ) in items" >  
                                                            ...
                                                            <tr  :key="item.id" :class="index" > 
                                                                ...
                                                                <td :rowspan="rowVisible(index,item,'kra')" v-if="filterForm.show_kra==1 && (items[index > 0 ? index - 1 : 0 ].kra_id != item.kra_id || index ==0)" >{{ item.krajoin ? item.krajoin.kra_name : '' }} <strong>(T-{{kraTotalTarget(item.kra_id ,'target')}}/A-{{kraTotalTarget(item.kra_id ,'achievement')}})</strong></td>
                                                                <td :rowspan="rowVisible(index,item,'kra')" v-if="filterForm.show_kra==1 && (items[index > 0 ? index - 1 : 0 ].kra_id != item.kra_id || index ==0)" >{{ item.krajoin ? item.krajoin.kra_weight : '' }} </td>
                                                                <td :rowspan="rowVisible(index,item,'kpi')" v-if="filterForm.show_kpi==1 && (items[index > 0 ? index - 1 : 0 ].kpi_id != item.kpi_id || index ==0)" >{{ item.kpijoin ? item.kpijoin.kpi_name : '' }} (T-{{kpiTotalTarget(item.kpi_id ,'target')}}/A-{{kpiTotalTarget(item.kpi_id ,'achievement')}})</td>
                                                                <!-- <td v-if="filterForm.show_mos==1">{{ item.mos_name }}  ({{ Number(moduleTotal(item)).toFixed(2) }}) </td> -->
                                                                <td v-if="filterForm.show_mos==1">{{ item.mos_name }} (W-{{item.weightage}}) (T-{{mosTotalTarget(item ,'target')}}/A-{{mosTotalTarget(item ,'achievement')}}) </td>  
                                                                <td v-bind:class=" targetTotal(item) >  100 ? 'gb_color_green': targetTotal(item) <  100  && targetTotal(item) > 0 ? 'gb_color_yellow' : ''">{{ Number(targetTotal(item)).toFixed(2)}}{{item.isvalorper == 1  ? '%' : ''}}</td>
                                                                <td v-bind:class=" achievementjoinTotal(item) >  100 ? 'gb_color_green': achievementjoinTotal(item) <  100  && achievementjoinTotal(item) > 0 ? 'gb_color_yellow' : ''">{{ Number(achievementjoinTotal(item)).toFixed(2) }}{{item.isvalorper == 1 ? '%' : ''}} </td>
                                                                <td v-bind:class=" achievementTotal(item ,targetTotal(item), achievementjoinTotal(item)) >  100 ? 'gb_color_green': achievementTotal(item ,targetTotal(item), achievementjoinTotal(item)) <  100  && achievementTotal(item ,targetTotal(item), achievementjoinTotal(item)) > 0 ? 'gb_color_yellow' : ''" v-if="filterForm.show_yachi==1">{{achievementTotal(item ,targetTotal(item), achievementjoinTotal(item))}}%</td>
                                                                ...
                                                                <td>  
                                                                    <div class="dropup">
                                                                        ...
                                                                        <div class="dropdown-menu dropdown-menu-right"> 
                                                                            ...
                                                                            <router-link traget="_blank" v-if="role_id == 5 || role_id == 6" class="dropdown-item" :to="{ path: '/measure_of_success/'+ item.kpi_id }"  ><i class="bx bx-edit-alt mr-1"></i> MOS Edit </router-link> 
                                                                        </div>
                                                                    </div> 
