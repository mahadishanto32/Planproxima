...
<!--                        <div class=" col-sm-3">
            ...
            <div class="content-body">
                ...
                <!--                         <div class="col-sm-3 col-lg-3" >
                                            <label for="users-list-verified">Report Name</label>
                                            <fieldset class="form-group">
                                                <select class="form-control"  v-model="updateForm.permission_for" @change="getItems(updateForm.permission_for)" id="users-list-verified" >
                                                    <option value=0>Monthly Summery Report Permission</option>
                                                    <option value=1>KRA KPI Report Settings(ADD ,EDIT & DELETE)</option>
                                                </select>
                                            </fieldset>
                                        </div> -->
                                            ...
                                            <table class="table table-striped">
                                                        ...
                                                        <th>
                                                            <div v-if="checked_all == true" class="row">
                                                                <div   class="col-12 col-sm-6 ">
                                                                    ...
                                                                    <fieldset class="form-group"> 
                                                                        <datepicker  @closed="startDateClosedFunction"   v-model="start_date" name="start_date" class="form-control"  ></datepicker> 
                                                                    </fieldset>
                                                                </div>
                                                                <div   class="col-12 col-sm-6  ">
                                                                    <label class="control-label">End Date </label>
                                                                        <fieldset class="form-group"> 
                                                                            <datepicker  @closed="endDateClosedFunction"   v-model="end_date" name="end_date" class="form-control"  ></datepicker>
                                                                        </fieldset>
                                                                </div> 
                                                            </div>
                                                        </th>
                                                ...
                                                <tbody>
                                                    <tr v-for="(row ,index) in items" :key="row.id">
                                                        <td>{{ index + 1 }}</td>  
                                                        <td>{{ row.name }}</td>  
                                                        <td v-if="row.monthly_date_range">
                                                            <div class="row  ">
                                                                <div   class="col-12 col-sm-6 ">
                                                                    <label class="control-label"> Start Date </label>
                                                                     <fieldset class="form-group"> 
                                                                        <datepicker  @closed="datepickerClosedFunction"  v-model="row.monthly_date_range.start_date" name="start_date" class="form-control"  ></datepicker> 
                                                                    </fieldset>
                                                                </div>
                                                                <div   class="col-12 col-sm-6  ">
                                                                    <label class="control-label">End Date </label>
                                                                        <fieldset class="form-group"> 
                                                                            <datepicker  @closed="datepickerClosedFunction"   v-model="row.monthly_date_range.end_date" name="end_date" class="form-control"  ></datepicker>
                                                                        </fieldset>
                                                                </div> 
                                                            </div>
                                                           
                                                        </td>
                                                        <td v-if="!row.monthly_date_range"></td>
                                                        <td v-if="row.monthly_date_range" >
...
export default {
  ...
  data() {
    return {
        ...
        updateForm: new Form({ 
           items : '', 
           permission_for :  this.$route.params.id,  
        }),
        state : {
                disabledDates: {
  ...
  created() {  
    ...
    this.getItems(this.permission_for);
  },
  methods: {
    check(){  
    ...
    update(){
       ...
       async getItems(permission_for){
           ...
           let loader = this.$loading.show();
           await axios.get(this.api_url + "monthly_date_range" + where, {
                    headers: {
                    "Content-Type": "application/json", 
                    Authorization: this.token ? `Bearer ${this.token}` : ""
                    },
